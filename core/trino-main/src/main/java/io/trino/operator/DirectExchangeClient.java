        ...
        implements Closeable
{
    ...
    public DirectExchangeClientStatus getStatus()
    {
        ...
        TDigest mergedResidualError = new TDigest();
        Map<TaskId, Long> zeros = new HashMap<>();
        long z = 0;
        for (HttpPageBufferClient client : allClients.values()) {
            pageBufferClientStatusBuilder.add(client.getStatus());
            mergedResidualError.mergeWith(client.getResidualErrorDistribution());
            z += client.zero;
            zeros.put(client.getRemoteTaskId(), client.zero);
        }
        ...
        synchronized (this) {
            return new DirectExchangeClientStatus(
                    ...
                    new TDigestHistogram(mergedResidualError),
                    z,
                    zeros);
        }
    }
