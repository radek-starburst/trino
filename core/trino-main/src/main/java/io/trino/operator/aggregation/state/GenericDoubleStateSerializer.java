        ...
        implements AccumulatorStateSerializer<GenericDoubleState>
{
    ...
    @Override
    public void serialize(@GroupId long groupId, GenericDoubleState state, BlockBuilder out)
    {
        if (state.isNull()) {
            out.appendNull();
        }
    ...
    @Override
    public void deserialize(@GroupId long groupId, Block block, int index, GenericDoubleState state)
    {
        state.setNull(false);
