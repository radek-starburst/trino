        ...
        implements AccumulatorStateSerializer<GenericBooleanState>
{
    ...
    @Override
    public void serialize(@GroupId long groupId, GenericBooleanState state, BlockBuilder out)
    {
        if (state.isNull()) {
            out.appendNull();
        }
    ...
    @Override
    public void deserialize(@GroupId long groupId, Block block, int index, GenericBooleanState state)
    {
        state.setNull(false);
